...
module Entry :
  sig
    ...
    val reset : Reason_js.reason -> string -> t -> t
    val is_lex : t -> bool
  end
module KeyMap : MyMap.S with type key = Key.t
type var_scope_kind = Ordinary | Async | Generator | Module | Global
...
val update_refis : (KeyMap.key -> refi_binding -> refi_binding) -> t -> unit
val add_refi : KeyMap.key -> refi_binding -> t -> unit
val remove_refi : KeyMap.key -> t -> unit
val get_refi : KeyMap.key -> t -> refi_binding option
val havoc_refi : KeyMap.key -> t -> unit
val filter_refis_using_propname : string -> 'a KeyMap.t -> 'a KeyMap.t
val havoc_refis : ?name:string -> t -> unit
val havoc : t -> unit
val reset : Reason_js.reason -> t -> unit
val is_lex : t -> bool
