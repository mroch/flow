...
(* Returns number of dependency edges added. *)
external update_dep_table_sqlite_c : string -> string -> bool -> int = "hh_update_dep_table_sqlite"
...
(Config : ConfigType) : CacheType with type key := Key.t and type value := Config.value = struct
  let string_of_key _key = failwith "OrderedCache does not support 'string_of_key'"
